<template>
  <div>
    ...
    <Drawer title="DML语句快速提交" v-model="latch.drawer" width="700">
      <Form :rules="ruleValidate" ref="formItem" :model="formItem">
        ...
        <FormItem label="审核人:" prop="assigned">
          <Select v-model="formItem.assigned" filterable>
            <Option v-for="i in fetchData.assigned" :value="i" :key="i">{{
              i
            }}</Option>
          </Select>
        </FormItem>
      ...
      <Form :label-width="30">
        <FormItem>
          <Button type="error" icon="md-trash" @click.native="clearForm()"
            >清除
          </Button>
          <Button
            type="primary"
          ...
          <Button type="warning" @click="beauty" class="margin-left-10"
            >美化</Button
          >
          <Button
            type="success"
...
export default class query_sql extends Mixins(fetch_mixin, FetchMixins) {
  ...
  commitOrder() {
    ...
    is_validate.validate((valid: boolean) => {
      if (valid) {
        ...
        PostOrder(order).finally(() => (this.validate_gen = true));
      }
    });
  }
    ...
    if (vl.expand === true) {
      ...
      CommonGetApis("fetch_table", { title: vl.title, source: this.source })
        .then((res: AxiosResponse<Res>) => {
          ...
          modules_order.changed_wordList(
            this.wordList.concat(res.data.payload.highlight)
          );
  ...
  mounted() {
    ...
    CommonPutApis("fetch_base", { source: this.source }).then(
