        ...
        >
          <FormItem label="环境:" prop="idc">
            <Select v-model="sql_order.idc" @on-change="fetchDiffSource">
              <Option v-for="i in fetchData.idc" :key="i" :value="i">{{
                i
              }}</Option>
            </Select>
          </FormItem>
          ...
          <FormItem label="">
            <Button @click="handleSubmit" style="width: 100px" type="primary"
              >提交</Button
            >
          </FormItem>
        </Form>
...
export default class work_flow extends Mixins(QueryMixin) {
  stepData = {
    ...
    describe: `欢迎你！ ${localStorage.getItem("user")}`,
  };
  stepList1 = [
    {
      title: "提交",
  ...
  stepRules = {
    text: [{ required: false, message: "请填写查询说明", trigger: "blur" }],
    idc: [
      {
        required: true,
        message: "环境地址不得为空",
  ...
  handleSubmit() {
    ...
    is_validate.validate((valid: boolean) => {
      if (valid) {
        CommonPostApis("refer", this.sql_order).then(() => {
