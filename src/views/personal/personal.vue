<template>
  <div>
    <Form ref="userForm" label-position="right">
      ...
      <FormItem>
        <Button type="warning" size="small" @click="show_edit_password"
          >{{ $t("general.change_password") }}
        </Button>
        <Button
          type="primary"
          size="small"
          @click="openMailChange"
...
export default class Personal extends Mixins(Basic) {
  ...
  saveEmail() {
    CommonChangeEmail(this.editEmailForm).finally(() => {
      localStorage.setItem("real_name", this.editEmailForm.real_name);
    });
  }

  init() {
    DashPutApi("profile").then((res: AxiosResponse<Res>) => {
