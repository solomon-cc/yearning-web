      ...
      <Table border :columns="columns" :data="table_data" stripe size="small">
        <template slot-scope="{ row }" slot="action">
          <template v-if="row.status !== 5">
            <Button type="success" @click="orderDetail(row)" size="small" ghost>
              详情
            </Button>
            <Poptip
              confirm
            ...
            >
              <Button
                type="error"
                v-if="row.status === 3 && row.delay !== 'none'"
                size="small"
                ghost
...
export default class platform_audit extends Mixins(Basic) {
  ...
  delayKill(vl: { work_id: string }) {
    AuditKillOrder({ work_id: vl.work_id }).then(() => {
      this.current_page();
    });
  }
