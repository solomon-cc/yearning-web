    ...
    <Col span="5">
      <Card>
        ...
        <CustomForm
          :item="general"
          :rule="ruleInline"
          :label-value="dbLabelValue"
          ref="db"
          :label-width="100"
        >
          <template slot="idc">
            <Select v-model="general.idc">
              <Option v-for="list in idcList" :value="list" :key="list">{{
                list
              }}</Option>
            </Select>
          </template>
        ...
        <Button type="success" @click="add_db()" style="margin-left: 5%"
          >确定</Button
        >
        <Button
          type="warning"
          @click="restCustomFields('db')"
          style="margin-left: 5%"
          >重置</Button
        >
      </Card>
        ...
        <div class="edit-table-con-1">
          <Table :columns="columns" :data="table_data">
            <template slot-scope="{ row }" slot="is_query">
              ...
              <Tag checkable color="success" v-else-if="row.is_query === 1"
                >读</Tag
              >
...
export default class database_manager extends Mixins(Basic) {
  ...
  add_db() {
    ...
    if (verify.isOk) {
      DBCreateOrEditApi({ tp: "create", db: this.general }).finally(() => {
  ...
  current_page(vl = 1) {
    DBFetchApi({ page: vl, find: this.find }).then(
  ...
  modifyBase() {
    ...
    DBCreateOrEditApi({ tp: "edit", db: x }).finally(() =>
      this.current_page(this.current)
    );
  }
