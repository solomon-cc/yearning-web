  ...
  <Row>
    <Col span="5">
      <Card>
        ...
        <CustomForm
          :item="userinfo"
          :rule="userInfoValidate"
          :label-value="userLabelValue"
          ref="reg"
        >
          <template slot="rule">
    ...
    <Col span="19" class="padding-left-10">
      <Card>
        ...
        <Form>
          ...
          ></Input>
          <Button @click="queryData" type="primary" class="margin-left-10"
            >查询</Button
          >
          <Button @click="queryCancel" type="warning" class="margin-left-10"
            >重置</Button
          >
        </Form>
    ...
    <Modal v-model="is_depend" title="依赖清单">
      ...
      <Tag
        type="border"
        v-for="i in depend_list.grained"
        :key="`grained-${i.name}`"
        >{{ i.name }}</Tag
      >
      <div slot="footer">
        <Button type="error" @click="del_user" :disabled="del_disabled"
          >删除</Button
        >
      </div>
    </Modal>
    ...
    <RulesLimits
      is_admin
      v-model="is_open"
      @success="current_page(current)"
    ></RulesLimits>
    <EditProfile
      v-model="is_edit"
      @call="current_page(current)"
      :user_info="payload"
    ></EditProfile>
  </Row>
</template>
...
@Component({
  components: { ChangePassword, RulesLimits, EditProfile, CustomForm },
})
export default class user_info extends Mixins(Basic) {
  ...
  registered() {
    ...
    if (verify.isOk) {
      this.loading = true;
      UserCreateOrEditApi({ tp: "create", user: this.userinfo }).finally(() => {
  ...
  show_depend(row: { username: string }) {
    ...
    UserDependGetApi({ tp: "depend", user: row.username })
      ...
      .finally(() => {
        if (
          this.depend_list.grained.length === 0 &&
          this.depend_list.source.length === 0
        ) {
  ...
  del_user() {
    UserDeleteApi(this.username).finally(() => {
      this.current_page(
        this.table_data.length === 1 ? this.current - 1 : this.current
      );
      this.is_depend = !this.is_depend;
    });
  }
