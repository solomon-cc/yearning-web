<template>
  <div>
    <Card dis-hover>
      <Row>
        <Col span="5"
          ><h2>工单编号:{{ this.order.work_id }}</h2></Col
        >
        <Col span="5" offset="14">
          <ButtonGroup>
            ...
            >
              <Button
                type="default"
                v-if="order.status === 2 && !JSON.parse($route.query.isAdmin)"
                >工单撤销
              </Button>
            </Poptip>
          </ButtonGroup>
    ...
    <Card dis-hover>
      ...
      <Steps :current="order.current_step">
        <Step
          v-for="i in order_step"
          :key="i.title"
          :title="i.desc"
          :content="`相关人员:${i.auditor}`"
        ></Step>
      </Steps>
    </Card>
    <br />
    ...
    <Card>
      ...
      <br />
      <template
        v-if="order.assigned === user && JSON.parse($route.query.isAdmin)"
      >
        <Testing></Testing>
      </template>
    </Card>
...
@Component({
  components: { collapse, basic, postForm, Testing, reject, StepDetail },
})
export default class profile extends Mixins(OrderProfileMixins) {
  ...
  created() {
    TplFetchProfile(this.order.idc as string).then(
