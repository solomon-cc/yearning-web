<template>
  <div>
    ...
    >
      <template v-if="!is_record">
        <template v-if="is_order">
          <Radio v-for="i in order_state" :key="i.key" :label="i.key">{{
            i.title
          }}</Radio>
        </template>
        <template v-else>
          <Radio v-for="i in query_state" :key="i.key" :label="i.key">{{
            i.title
          }}</Radio>
        </template>
      </template>
    ...
    <DatePicker
      ...
      type="datetimerange"
      placeholder="请选择查询的时间范围"
      v-model="find.picker"
      @on-change="searchChange"
      style="width: auto; margin-left: 1%"
      :editable="false"
    ></DatePicker>
    <Input
      suffix="ios-search"
